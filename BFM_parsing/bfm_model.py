"""
Acknowledgement: 
Code modified from: https://github.com/ascust/3DMM-Fitting-Pytorch/blob/master/core/models.py
"""
...
class ReconModel(nn.Module):
    ...
    def get_renderer(self, device):
        ...
        ) # we need to use the rasterizer to derive the visibility map
    ...
    def Split_coeff(self, coeff):
        id_coeff = coeff[:, :199]  # identity(shape) coeff of dim 199
        ex_coeff = coeff[:, 199:278]  # expression coeff of dim 79
        tex_coeff = coeff[:, 278:477]  # texture(albedo) coeff of dim 199
        angles = coeff[:, 477:480]  # ruler angles(x,y,z) for rotation of dim 3
        gamma = coeff[:, 480:507]  # lighting coeff for 3 channel SH function of dim 27
        translation = coeff[:, 507:]  # translation coeff of dim 3
    ...
    def Projection_block(self, face_shape):
        ...
        ## FIXED (solved the problem of not able to use size other than 224)
    ...
    def forward_shape(self, coeff):
        # this function is to get the projected shape
        batch_num = coeff.shape[0]
        id_coeff, ex_coeff, tex_coeff, angles, gamma, translation = self.Split_coeff(coeff)
        face_shape = self.Shape_formation(id_coeff, ex_coeff)
        rotation = self.Compute_rotation_matrix(angles)
        face_shape_t = self.Rigid_transform_block(face_shape, rotation, translation)
        face_shape_proj = self.Projection_block(face_shape_t)
        return face_shape_proj
