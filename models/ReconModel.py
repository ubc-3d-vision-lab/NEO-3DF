...
https://github.com/ascust/3DMM-Fitting-Pytorch/blob/master/core/BaseModel.py 
"""
...
class ReconModel(nn.Module):
    ...
    def forward_customized(self, face_shape, face_texture, angles, translation, ex_coeff, gamma):
        # this function is to render a given face shape and texture
        batch_num = face_shape.shape[0]
        face_shape = face_shape + torch.reshape(torch.einsum('ij,aj->ai', self.exBase, ex_coeff), (batch_num, -1, 3))
    ...
    def forward(self, coeff, custom_texture=None):
        ...
        if custom_texture is not None:
            face_color = TexturesVertex(torch.tensor(np.reshape(custom_texture*255., (1,-1,3)),
                                                     dtype=torch.float32,
                                                     requires_grad=False, 
                                                     device='cuda'))
