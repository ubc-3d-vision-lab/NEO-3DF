...
def get_part_colored(part_verts):
    colors = np.ones([part_vertices['S_overall'].shape[0], 3], dtype=np.float32) / 2
    for i in part_verts:
        colors[i,:] = np.array([0,0.8,0.8])
    return colors#*255
seg_colors_parts = {
    'Overall': seg_colors,
    'Nose': get_part_colored(part_vertices['S_nose']),
    'Eyebrows': get_part_colored(part_vertices['S_eyebrows']),
    'Eyes': get_part_colored(part_vertices['S_eyes']),
    'Lips': get_part_colored(list(part_vertices['S_llip'])+list(part_vertices['S_ulip']))
}
